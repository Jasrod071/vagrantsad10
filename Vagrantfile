...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "gw" do |gw|
    ...
    gw.vm.provision "shell", path: "gw/provision.sh", env: { "LDAP_PASS" => "adeasysl" }
  ...
  config.vm.define "idp" do |idp|
    ...
    idp.vm.provision "shell", run: "always", inline: "ip route del default && ip route add default via 172.2.#{N}.1"        
  ...
  # 5. Proxy en DMZ (El que ya tenías)
  ...
  # 7. NUEVA MÁQUINA SQUID
  config.vm.define "squid" do |squid|
    squid.vm.box = "bento/ubuntu-24.04" 
    squid.vm.hostname = "squid-#{iniciales}"
    squid.vm.network "private_network", ip: "172.2.#{N}.5", netmask: "255.255.255.0", virtualbox__intnet: "red_lan"
